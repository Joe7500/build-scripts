...
repo sync --force-sync --optimized-fetch --no-clone-bundle --no-tags  -j 4 -n

main() {
    # Run repo sync command and capture the output
    find .repo -name '*.lock' -delete
    repo sync -c -j1 --force-sync --no-clone-bundle --no-tags --prune -n 2>&1 | tee /tmp/output.txt
 ...
 if ! grep -qe "Failing repos:\|uncommitted changes are present" /tmp/output.txt ; then
    ...
    find .repo -name '*.lock' -delete
    repo sync -c -j1 --force-sync --no-clone-bundle --no-tags --prune -n
}
